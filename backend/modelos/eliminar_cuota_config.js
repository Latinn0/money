		...
		if (arrays.length==2){
			...
			jwt.verify(arrays[0], 'clWve-G*-9)1', function(err, decoded) {
				if (err) {
					...
					/*liso las cuotas y elimino la que necesito eliminar y ya*/
					
					redisClient.keys('cuotas_estaticas_'+decoded.d+'_'+arrays[1]+"_*",function(err3,reply3){
						if(reply3.length > 0){
							redisClient.del(reply3[0],function(er,repl){
								resolve([true,true]);
							});
						}else{
							reject([false,"4"]);
