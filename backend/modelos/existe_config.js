		...
		if (arrays.length==1){
			...
			jwt.verify(arrays[0], 'clWve-G*-9)1', function(err, decoded) {
				if (err) {
					...
					/*
					verifica si extrie configuracion se simple en bd y devuelve
					*/
					
					redisClient.keys("simple_config_*_"+decoded.i,function(err,reply) {
						if(reply!==null){
							redisClient.get(reply[0],function(errs,replys) {
								resolve([true,replys]);
							});
						}else{
							reject([false,"4"]);
